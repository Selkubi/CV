...
create_CV_object <- function(data_location,
  ...
  if (is_google_sheets_location) {
    ...
    cv$otjer_skills <- readr::read_csv(paste0(data_location, "other_skills.csv"), skip = 1)
...
#' @param out_of The relative maximum for skills. Used to set what a fully filled in skill bar is.
print_skill_bars_other <- function(cv, out_of = 5, bar_color = "#969696", bar_background = "#d9d9d9", glue_template = "default") {
  if (glue_template == "default") {
    glue_template <- "
<div
class = 'skill-bar'
